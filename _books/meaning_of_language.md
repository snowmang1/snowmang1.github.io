---
layout: book-review
title: The meaning of language
author: Heidi Savage, Melissa Ebbers, Robert M. Martin
cover: assets/img/book_covers/meaning-of-language.jpeg
olid: # use Open Library ID to fetch cover (if no `cover` is provided)
isbn: 9780262535731 # use ISBN to fetch cover (if no `olid` is provided, dashes are optional)
categories: non-fiction language
tags: language
buy_link: https://mitpress.mit.edu/9780262535731/the-meaning-of-language/
started: 2023-01-01
finished:
released: 2020
stars: 4
goodreads_review:
status: reading
---

Fantastic book teaching one how to think about language.
